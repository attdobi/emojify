...
class emoji_lib:
	...
	def filter_emoji(self, word='dog', face_filter='off', pattern_type='single', user_lang='all', date_range='all'):
		...
		date_sel = self.parse_date(date_range)
		# TODO: Speed up this query into a single step.
		#ID_RANGE = 1000000
		#date_sel = 'order by id DESC limit %s' % ID_RANGE
		########## If the face filter is OFF: ################
		if face_filter == 'off':
	...
	def sample_art(self):
		...
		self.cur.execute("SELECT b.text from (SELECT * from emoji_tweet WHERE id BETWEEN {:d} AND {:d} ) as a join tweet_dump b on a.tweet_id=b.id WHERE (a.emojiCountSum > 30) limit 50;".format(rand_start, rand_end))
	...
	def sample_art_options(eCount='>=30', eTypes='>=0', eStrTypes='>=0', ePatTypes='>=0', NL='>=0'):
		...
		last_id=self.get_last_id()
		self.cur.execute("SELECT text from emoji_tweet WHERE (emojiCountSum {:s} AND emojiTypes {:s} AND emojistrTypes {:s} AND emojiPatternTypes {:s} AND newlineCount {:s}) order by random() limit 50;".format(eCount, eTypes, eStrTypes, ePatTypes, NL))
