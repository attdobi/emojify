...
if len(sys.argv) == 2:
	...
	cores=4
	print('running on core {:d} of 4'.format(core_number+1))
else:
	#run on 1 core
...
if __name__ == "__main__":
	while run:
		conn = psycopg2.connect("host=172.31.22.77 port=5432 dbname=emoji_db user=postgres password=darkmatter")
		cur = conn.cursor()
		cur.execute("SELECT tweet_id from has_emoji WHERE MOD(tweet_id,4)=%s order by tweet_id DESC limit 1;",(core_number,))#find last processed id
		last_id=cur.fetchone()
		cur.execute("SELECT * from tweet_dump WHERE (id>%s AND MOD(id,4)=%s) LIMIT 10000;",(last_id,core_number)) 
